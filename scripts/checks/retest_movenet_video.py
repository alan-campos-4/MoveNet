...
def detect_pose_sequence():
	# Load the GIF
	gif = cv2.VideoCapture(gstreamer_pipeline(0), cv2.CAP_GSTREAMER)
	frames = []
	...
	for frame in frames:
		...
		frame_resized = tf.image.resize_with_pad(tf.expand_dims(frame_rgb, axis=0), 256, 256) # 192 for lightning
...
def visualize_and_create_pose_sequence(keypoints_list, output_gif_path, default_fps=10):
	# Load the GIF
	...
	for frame_index, (frame, keypoints) in enumerate(zip(gif, keypoints_list)):
		...
		if keypoints.shape == (1, 1, 17, 3):
			...
			for connection in connections:
				start_point = (int(keypoints[connection[0], 1] * frame.shape[1]), int(keypoints[connection[0], 0] * frame.shape[0]))
				end_point =   (int(keypoints[connection[1], 1] * frame.shape[1]), int(keypoints[connection[1], 0] * frame.shape[0]))
